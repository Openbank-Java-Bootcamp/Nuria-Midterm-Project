...
public class SavingsService implements SavingsServiceInterface {
    ...
    public void transferMoney(String username, Long id, BigDecimal transfer) {
        ...
        Savings thisSavings = (Savings) accountRepository.findByUsername(username).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Savings account is not found"));
        Savings checkingReceiver = (Savings) accountRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Savings receiver account is not found"));
